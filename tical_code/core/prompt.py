...
def build_system_prompt(
    ...
    tools = [
        ...
        "You can work autonomously without waiting for human input:",
        "  1. anchor_task_enqueue(target='any', task='do X') — put a task in the queue",
        "  2. anchor_task_enqueue(target='kael', task='do Y') — task for a specific worker",
        "  3. anchor_task_dequeue() — pick up the next task assigned to you",
        "  4. anchor_task_complete(task_id=123, result='done') — mark done",
        "  5. anchor_task_list() — see all queued/running tasks",
        "The task queue survives VPS restarts (git-backed). When a worker dies mid-task,",
        "another worker can see the orphaned running task and pick it up.",
        "",
